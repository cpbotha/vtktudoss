...
#if !defined(__geom_CellXYBinarySearchZ_ipp__)
#error This file is an implementation detail of the class CellXYBinarySearchZ.
#endif
...
inline
typename CellXYBinarySearchZ<RecordType, MultiKeyType, KeyType,
			     BoundarySpecialization>::size_type
CellXYBinarySearchZ<RecordType,MultiKeyType,KeyType,BoundarySpecialization>::
window_query(OutputIterator iter, const bbox_type& window) const
{
  ...
  typename cell_type::const_iterator record_container_end;
  ...
  if (BoundarySpecialization) {
	...
	record_container_end = record_container.end();

	// Binary search to find the beginning of the records in window.
	record_iter = record_container.search(window.getLowerCorner());
	...
	if (i == istart) {
	  if (i == istop) {
		...
		for (; record_iter != record_container_end && 
